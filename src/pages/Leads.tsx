...
export default function Leads() {
  ...
  const handleConvertToOpportunity = (lead: Lead) => {
    ...
    log("convert", "lead", lead.name, `Converted to opportunity`, lead.id.toString());
    // Update lead status
    setLeads(leads.map(l =>
      l.id === lead.id ? { ...l, status: "Qualified" } : l
    ));
  };
  ...
  return (
    <AppLayout title="Leads">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => handleLogActivity(lead)}>
      ...
      <LogActivityDialog
        ...
        defaultAccount={selectedLeadForActivity?.company || ''}
        onSave={(activity) => {
          toast.success("Activity logged", {
            description: `${activity.type} logged for ${selectedLeadForActivity?.name}`,
          });
        }}
      />
    </AppLayout>
